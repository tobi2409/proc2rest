...
export function createFetchClientFunction(routeStub) {
    try {
        ...
        throw new Error(`Failed to create fetch client function for '${routeStub.functionName}': ${error instanceof Error ? error.message : error}`)
    }
}

export function createFetchClientFileContent({ generatorConfig, clientFunctionsBlock }) {
    try {
        ...
        throw new Error(`Failed to create fetch client file content: ${error instanceof Error ? error.message : error}`)
    }
}
