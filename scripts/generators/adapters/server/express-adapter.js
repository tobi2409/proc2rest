...
export function createExpressRouteBody(routeStub) {
    try {
        ...
        throw new Error(`Failed to create express route body for '${routeStub.functionName}': ${error instanceof Error ? error.message : error}`)
    }
}

export function createExpressRoutesFileContent({ generatorConfig, importsBlock, rawCode, routesCode }) {
    try {
        ...
        throw new Error(`Failed to create express routes file content: ${error instanceof Error ? error.message : error}`)
    }
}
