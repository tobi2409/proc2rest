...
function generateRoutesFile(serverAdapter) {
    try {
        ...
        for (const serverConfig of configuredServers) {
            ...
            const serverFileName = getRelativePathFromSrcDir(
                absoluteServerFile,
                srcServerRootPath,
            );
            ...
            importsLines.push(
                `import * as ${serverNamespace} from './${importPath}'`,
            );
        }
        ...
        const rawCode = rawServerFiles
            .map((rawFile) => {
                ...
                if (!fs.existsSync(rawFilePath)) {
                    throw new Error(
                        `Could not find raw server file ${rawFile}`,
                    );
                }
