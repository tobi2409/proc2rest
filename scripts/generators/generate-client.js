...
import {
    getAppRootPath,
    getGeneratedClientRootPath,
    getSrcClientRootPath,
    getSrcServerRootPath,
...
function createApiClientFiles(
    ...
    try {
        ...
        for (const serverConfig of servers) {
            ...
            const relativeServerFile = getRelativePathFromSrcDir(
                path.join(appRootPath, serverSrc),
                srcServerRootPath,
            );
        ...
        console.error(
            "Error creating API client files:",
...
function addApiClientImports() {
    try {
        ...
        for (const clientFileName of jsClients) {
            if (
                typeof clientFileName !== "string" ||
                clientFileName.length === 0
            ) {
                throw new Error(
                    "Each jsClient entry must be a non-empty string",
                );
            }

            const copiedClientPath = path.join(
                destDir,
            ...
            if (!fs.existsSync(copiedClientPath)) {
                throw new Error(
                    `Client file not found in generated directory: ${copiedClientPath}`,
                );
            }

            const sourceClientContent = fs.readFileSync(
                copiedClientPath,
                "utf8",
            );
            const serverImportRegex =
                /^\s*\/\/\s*@server-import\s+([^\s]+)\s*$/gm;
            const importLines = [
                ...sourceClientContent.matchAll(serverImportRegex),
            ].map(([, serverFileName]) => {
                const serverNamespace = serverNamespaceBySrc[serverFileName];
                if (!serverNamespace) {
                    throw new Error(
                        `Server '${serverFileName}' not found in config`,
                    );
                }
                return `import * as ${serverNamespace} from './${serverFileName.replace(".ts", "")}-client.js'`;
            });

            const contentWithoutMarkers = sourceClientContent
                .replace(serverImportRegex, "")
                .trimStart();
            const importsBlock = importLines.join("\n");
            const copiedClientContent =
                importsBlock.length > 0
        ...
        console.error(
            "Error adding API client imports:",
            error instanceof Error ? error.message : error,
        );
        process.exit(1);
    }
