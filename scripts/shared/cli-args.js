...
function getCliArgValue(flagName) {
    try {
        ...
        throw new Error(`Failed to get CLI arg value for '${flagName}': ${error instanceof Error ? error.message : error}`)
    }
}
...
export function getAppRootPath() {
    try {
        ...
        throw new Error(`Failed to get app root path: ${error instanceof Error ? error.message : error}`)
    }
}
...
export function getConfigPath(appRootPath = getAppRootPath()) {
    try {
        ...
        throw new Error(`Failed to get config path: ${error instanceof Error ? error.message : error}`)
    }
}

export function getSrcServerRootPath(appRootPath = getAppRootPath()) {
    return resolveGeneratedPath(appRootPath, getCliArgValue('--srcServerDir'), 'src/server')
}

export function getSrcClientRootPath(appRootPath = getAppRootPath()) {
    return resolveGeneratedPath(appRootPath, getCliArgValue('--srcClientDir'), 'src/client')
}
